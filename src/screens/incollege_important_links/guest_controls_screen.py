...
def handle_user_selection(user_selection):
  ...
  if auth.logged_in_user:
    user_id = auth.logged_in_user.get('unique_id')
...
def get_user_nofication_options():
  options = []
  user = userController.get_user_by_id(auth.logged_in_user['unique_id'])
  if user['sms_notifications'] == False:
  ...
  if user['ad_notifications'] == False:
